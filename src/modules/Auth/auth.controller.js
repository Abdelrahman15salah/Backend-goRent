...
const login = async (req, res, next) => {
  try {
    ...
    if (!email || !password) {
      return next(
        new Error("يرجى إدخال البريد الإلكتروني وكلمة المرور للمتابعة.", {
          cause: 400,
        }),
      );
    }
    ...
    if (!isMatch) {
      return next(
        new Error("بيانات الدخول غير صحيحة، يرجى المحاولة مرة أخرى.", {
          cause: 400,
        }),
      );
    }
...
const register = async (req, res, next) => {
  try {
    ...
    if (!name || !email || !password) {
      return next(
        new Error(
          "يرجى إدخال جميع البيانات المطلوبة (الاسم، البريد، كلمة المرور).",
          { cause: 400 },
        ),
      );
    }
...
const getSocketToken = async (req, res, next) => {
  try {
    ...
    if (!token) {
      return next(
        new Error("عذراً، يجب تسجيل الدخول للقيام بهذا الإجراء.", {
          cause: 401,
        }),
      );
    }
...
const verifyOTP = async (req, res, next) => {
  try {
    ...
    if (!email || !otp) {
      return next(
        new Error("يرجى إدخال البريد الإلكتروني ورمز التحقق.", { cause: 400 }),
      );
    }
    ...
    if (!validOtp) {
      ...
      return next(
        new Error("رمز التحقق غير صحيح أو منتهي الصلاحية، يرجى طلب رمز جديد.", {
          cause: 400,
        }),
      );
    }
...
const resetPassword = async (req, res, next) => {
  try {
    ...
    if (!email || !otp || !newPassword) {
      ...
      return next(
        new Error("يرجى تعبئة جميع الحقول المطلوبة لتغيير كلمة المرور.", {
          cause: 400,
        }),
      );
    }
